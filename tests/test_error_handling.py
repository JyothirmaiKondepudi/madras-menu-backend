...
def test_deleting_referenced_menu_item_returns_409_not_500(client, admin_auth_headers):
    ...
    client.post(
        "/item-relationships", json={"childId": child["id"], "parentId": parent["id"]}, headers=admin_auth_headers
    )

    # parent is still referenced by the edge above — deleting it must fail
    # cleanly (409), not leak a raw IntegrityError as a 500
    resp = client.delete(f"/menu-items/{parent['id']}", headers=admin_auth_headers)
...
def test_duplicate_menu_item_name_returns_409(client, admin_auth_headers):
    body = {"name": "Unique Dish", "course": "main", "vegNonveg": "veg", "priceWeight": "standard"}
    first = client.post("/menu-items", json=body, headers=admin_auth_headers)
    assert first.status_code == 200

    second = client.post("/menu-items", json=body, headers=admin_auth_headers)
