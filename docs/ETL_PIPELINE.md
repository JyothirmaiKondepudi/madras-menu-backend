...
**Ollama is now installed and the real round trip is verified — not just the failure paths.** `nomic-embed-text` and `llama3.1:8b` are both pulled; Ollama runs as a `brew services` background daemon, using this machine's Apple M3 GPU via Metal. 7 real dishes are embedded (`Tamarind rice`, `Basmati rice`, `Curd Rice`, `Chicken Biryani`, `Gulab Jamun`, `Achar`, `Butter Naan`). The actual motivating test case — a semantic search for `"Tamarind infused rice with roasted peanuts and cashews and tempering"`, sharing almost no vocabulary with the real dish name — correctly returns `Tamarind rice` as the closest match (cosine distance 0.2512), meaningfully ahead of every other dish, with the remaining ranking also making real sense (other rice dishes cluster next-closest; dessert/condiment/bread land furthest). This is the first real, end-to-end proof that the whole design — the reason `menu_item_embeddings` exists at all — actually works.
