...
def get_projects(db: Session = Depends(get_db), current_user: User = Depends(get_current_user)):
    # admin sees every project; anyone else only sees projects they're
    # linked to via user_projects (current_user.projects) — the one
    # authorization rule this pass establishes. Finer per-role behavior
    # (staff vs. chef vs. client) isn't designed yet.
    if current_user.userRole == "admin":
        return get_all_projects(db)
    return current_user.projects

@router.post("/projects/{project_id}/users/{user_id}", response_model=ProjectOut)
def add_project_user(
    project_id: UUID, user_id: UUID, db: Session = Depends(get_db), admin: User = Depends(require_admin)
):
...
@router.patch("/projects/{project_id}/final-invoice/{invoice_id}", response_model=ProjectOut)
def set_project_final_invoice(
    project_id: UUID, invoice_id: UUID, db: Session = Depends(get_db), admin: User = Depends(require_admin)
):
...
def add_project(new_project: ProjectCreate, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
...
def update_project(
    project_id: UUID, updates: ProjectUpdate, db: Session = Depends(get_db), admin: User = Depends(require_admin)
):
...
def delete_project(project_id: UUID, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
