...
from auth.dependencies import get_current_user, require_admin, user_project_ids
...
def add_invoice(new_invoice: InvoiceCreate, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
...
def update_invoice(
    invoice_id: UUID, updates: InvoiceUpdate, db: Session = Depends(get_db), admin: User = Depends(require_admin)
):
...
def delete_invoice(invoice_id: UUID, db: Session = Depends(get_db), admin: User = Depends(require_admin)):
